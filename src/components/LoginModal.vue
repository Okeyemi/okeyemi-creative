<template> 
  <section
  @click="$emit(`close-login`)" 
  class="z-20 h-screen w-screen bg-gray-500 fixed top-0 opacity-50"
  ></section>
<div class="absolute inset-0">
    <div class="flex h-full">
        <div class="z-30 m-auto bg-white p-2 rounded shadow w-1/3">   
         <div class="p-2 border ">
        <h1 class="text-2xl text-center">Login</h1>
       <section class="my-5 text-center">
        <button  class="border px-2 rounded" @click="loginWithGoogle">Login With Google</button> 
       </section>
        <form class="p-2 my-2" @submit.prevent="submit" action="">
            <div class="my-4">
                <label for="">Email or Username</label>
                <input
                ref="emailRef"
                v-model="form.email"
                class="rounded shadow p-2 w-full" placeholder="Enter your email or username" />
            </div>
            <div>
                <label for="">Password</label>
                <input 
                v-model="form.password"
                class="rounded shadow p-2 w-full  "  type="password"  placeholder="Enter your password"/>
            </div>
            <div class="my-4">
                <button type="submit" class="w-full rounded-md bg-gradient-to-r from-red-800 to-pink-800 text-white p-2">Login</button>
            </div>
        </form>
        </div>
    </div>
</div>
</div>
</template>

<script>
    import firebase from "../Utilities/firebase"
export default{
    data(){
        return{
            form:{
                email:'',
                password:'',
            },
        };
    },
    mounted(){
this.$refs.emailRef.focus();
    },
    methods:{
        submit(){
            firebase.auth().signInWithEmailAndPassword
        },
    },
};

</script>

<style>

</style>